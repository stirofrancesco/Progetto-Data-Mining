...
#Lettura Matrice di espressione e creazione del grafo di correlazione
...
matrix <- data.matrix(expression)
matrix <- matrix[1:1000,1:100]
matrix <- t(matrix)
matrix <- matrix[,apply(matrix,2,sd)!=0]
coefGGM <- lassoGGM(matrix)
...
# 
# 
# vector <- colSds(matrix)
# 
# 
# 
# expression<-t(expression)
# expression <- expression[,1:800]
